...
export class MarkingReviewPolicyService {
    private readonly thresholds = [50, 60, 70, 80, 90];

    shouldRequireReview(parsed: LLMRawMarkingResponse, awarded: number, max: number): boolean {
        ...
        return this.thresholds.some(t => Math.abs(percentage - t) <= 5);
    }

    getReviewReason(parsed: LLMRawMarkingResponse, awarded: number, max: number): string {
        const reasons: string[] = [];

        if (parsed.review_reason) reasons.push(parsed.review_reason);
        if (parsed.confidence_score < 0.7) reasons.push('Low confidence score');
